    ...
    for i in range(num_state):
        ...
        for j, state in enumerate(states):
            if state == i:
                ...
                act_collector.append(-1)
        hist[i] = cal_percentage(np.array(act_collector), num_window)
    return hist


def draw_roaming_percentage(file_name, num_window=4, num_state=2):
    ...
    location_labels = ['Edge', 'Center']
    for i in range(num_state):
        x_0 = x - bar_width * num_state / 2
        x_plot = x_0 + i * bar_width + bar_width / 2
        plt.bar(x_plot, mean[i], width=bar_width, label=location_labels[i], color=plt.cm.viridis(i/num_state))
        # plt.errorbar(x_plot, mean[i], yerr=std[i], color='b')
...
def draw_location_distribution(file_name, num_state=2):
    data = np.load(file_name)
    states = data[0].astype(int)
    ...
    plt.figure(figsize=(8, 6))
    plt.stackplot(range(time_steps), np.transpose(loc_distrib), labels=['Edge', 'Center'])
    # plt.title('Untitled')
    plt.xlabel('Steps')
