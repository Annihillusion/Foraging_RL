...
def draw_location_distribution(file_name, num_state=3):
    ...
    plt.figure(figsize=(8, 6))
    plt.stackplot(range(time_steps), np.transpose(loc_distrib), labels=['Center', 'Mid', 'Edge'])
    plt.title('Untitled')
    plt.xlabel('Steps')
...
if __name__ == '__main__':
    # draw_location_distribution('train_rec.npy')
    draw_roaming_percentage('train_rec.npy')
